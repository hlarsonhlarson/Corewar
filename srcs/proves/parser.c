...
/*   Updated: 2019/12/28 16:44:05 by hlarson          ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
t_list		*carret_generate(t_list *files, unsigned char *map,
		size_t file_num, unsigned char *color)
{
	t_carret	carret;
	size_t		index;
	t_list		*result;
	...
	while (files)
	{
		...
		ft_memcpy(map + carret.pos, ft_jump(files->content,
					16 + (PROG_NAME_LENGTH) + (COMMENT_LENGTH)),
				carret.champion_exec_code_size);
		colorizer(color + carret.pos, (unsigned char)index,
				carret.champion_exec_code_size);
...
void	parser(t_list *args, t_parse *flags)
{
	...
	files = ft_lst_turn(files);
	flags->carret = carret_generate(files, flags->map,
			ft_lst_chain_len(files), flags->color);
